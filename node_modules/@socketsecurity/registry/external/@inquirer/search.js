...
function isUnicodeSupported() {
  ...
  return Boolean(process$2.env['WT_SESSION']) ||
  // Windows Terminal
...
const defaultTheme = {
  ...
  spinner: {
    interval: 80,
    frames: ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'].map(frame => yoctocolorsCjsExports.yellow(frame))
  },
  style: {
    ...
    key: text => yoctocolorsCjsExports.cyan(yoctocolorsCjsExports.bold(`<${text}>`))
  }
};
...
function usePrefix({
  ...
  useEffect(() => {
    if (status === 'loading') {
      ...
      const delayTimeout = setTimeout(async_hooks.AsyncResource.bind(() => {
        setShowLoader(true);
        tickInterval = setInterval(async_hooks.AsyncResource.bind(() => {
          inc = inc + 1;
...
function breakLines(content, width) {
  return content.split('\n').flatMap(line => wrapAnsiExports(line, width, {
    trim: false,
    hard: true
...
class ScreenManager {
  ...
  render(content, bottomContent = '') {
    ...
    if (bottomContentHeight > 0) output += ansiEscapesExports.cursorUp(bottomContentHeight);
    // Return cursor to the initial left offset.
    output += ansiEscapesExports.cursorTo(this.cursorPos.cols);
    /**
     * Render and store state for future re-rendering
     */
    this.write(cursorDown(this.extraLinesUnderPrompt) + ansiEscapesExports.eraseLines(this.height) + output);
 ...
 */
class Separator {
  separator = yoctocolorsCjsExports.dim(Array.from({
    length: 15
  }).join(figures.line));
  type = 'separator';
...
var index = createPrompt((config, done) => {
  const {
    pageSize = 7,
    validate = () => true
  } = config;
  ...
  return [[prefix, message, searchStr].filter(Boolean).join(' '), `${error ?? page}${helpTip}${choiceDescription}`];
});
