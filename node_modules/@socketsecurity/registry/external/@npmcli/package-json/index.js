...
var require$$5 = require('fs/promises');
...
var require$$1$7 = require('path/win32');
var require$$0$5 = require('module');
var require$$0$6 = require('process');
...
function requireLib$a() {
  ...
  const parseError = (e, txt, context = 20) => {
    ...
    } else /* istanbul ignore next - doesnt happen in Node 22 */if (msg.match(/^Unexpected end of JSON.*/i)) {
        errIdx = txt.length - 1;
      }
    if (errIdx == null) {
...
function requireCommonjs$1() {
  ...
  commonjs$1.PathScurry = commonjs$1.Path = commonjs$1.PathScurryDarwin = commonjs$1.PathScurryPosix = commonjs$1.PathScurryWin32 = commonjs$1.PathScurryBase = commonjs$1.PathPosix = commonjs$1.PathWin32 = commonjs$1.PathBase = commonjs$1.ChildrenCache = commonjs$1.ResolveCache = void 0;
   ...
   */
  commonjs$1.PathScurry = process.platform === 'win32' ? PathScurryWin32 : process.platform === 'darwin' ? PathScurryDarwin : PathScurryPosix;
  return commonjs$1;
}
...
function requireCommonjs() {
  if (hasRequiredCommonjs) return commonjs$3;
  ...
  (function (exports) {
    ...
    var minimatch_2 = requireCommonjs$3();
    Object.defineProperty(exports, "escape", {
      enumerable: true,
      get: function () {
  ...
  const open = (_args, opts = {}, extra = {}) => {
    ...
    if (platform === 'linux' && os.release().toLowerCase().includes('microsoft')) {
      ...
      if (!process.env.BROWSER) {
        return Promise.reject(new Error('Set the BROWSER environment variable to your desired browser.'));
      }
    }
...
function requireSort$1() {
  if (hasRequiredSort$1) return sort_1;
  hasRequiredSort$1 = 1;
...
function requireRevs() {
  ...
  revs = async (repo, opts = {}) => {
    ...
    const {
      stdout
    } = await spawn(['ls-remote', repo], opts);
...
function requireDevEngines() {
  ...
  const satisfies = requireSatisfies();
  const validRange = requireValid();
  const recognizedOnFail = ['ignore', 'warn', 'error', 'download'];
  ...
  function checkDependency(wanted, current, opts) {
    ...
    if (wanted.name !== current.name) {
      return new Error(`Invalid name "${wanted.name}" does not match "${current.name}" for "${engine}"`);
    }
    if (properties.includes('version')) {
      ...
      if (validRange(wanted.version)) {
        ...
        return new Error(`Invalid version "${wanted.version}" does not match "${current.version}" for "${engine}"`);
      }
    }
  }
...
function requireLib$2() {
  ...
  const pickManifest = (packument, wanted, opts) => {
    ...
    if (defaultVer && (range === '*' || semver.satisfies(defaultVer, range, {
      loose: true
    })) && !restricted[defaultVer] && !shouldAvoid(defaultVer, avoid)) {
    ...
    const entries = allEntries.filter(([ver]) => semver.satisfies(ver, range, {
      loose: true
    })).sort((a, b) => {
      const [vera, mania] = a;
      const [verb, manib] = b;
...
function requireClone() {
  ...
  const {
    parse
  } = require$$0; // eslint-disable-line node/no-deprecated-api
...
function requireIsClean() {
  ...
  const spawn = requireSpawn();
  isClean = (opts = {}) => spawn(['status', '--porcelain=v1', '-uno'], opts).then(res => !res.stdout.trim().split(/\r?\n+/).map(l => l.trim()).filter(l => l).length);
  return isClean;
}
...
function requireScan$1() {
  if (hasRequiredScan$1) return scan$1;
  hasRequiredScan$1 = 1;
  var licenses = [].concat(require$$1$1).concat(require$$1);
...
var spdxExpressionParse$1;
var hasRequiredSpdxExpressionParse$1;
function requireSpdxExpressionParse$1() {
  if (hasRequiredSpdxExpressionParse$1) return spdxExpressionParse$1;
  hasRequiredSpdxExpressionParse$1 = 1;
  var scan = requireScan$1();
...
function requireValidateNpmPackageLicense() {
  ...
  var parse = requireSpdxExpressionParse$1();
...
function requireLib() {
  ...
  class PackageJson {
    static normalizeSteps = Object.freeze(['_id', '_attributes', 'bundledDependencies', 'bundleDependencies', 'optionalDedupe', 'scripts', 'funding', 'bin']);

    // npm pkg fix
    static fixSteps = Object.freeze(['binRefs', 'bundleDependencies', 'bundleDependenciesFalse', 'fixNameField', 'fixVersionField', 'fixRepositoryField', 'fixDependencies', 'devDependencies', 'scriptpath']);
    static prepareSteps = Object.freeze(['_id', '_attributes', 'bundledDependencies', 'bundleDependencies', 'bundleDependenciesDeleteFalse', 'gypfile', 'serverjs', 'scriptpath', 'authors', 'readme', 'mans', 'binDir', 'gitHead', 'fillTypes', 'normalizeData', 'binRefs']);
